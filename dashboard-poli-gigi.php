...
try {
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action'])) {
        ...
        try {
            if ($action === 'update_item') {
                ...
                $stmt = $pdo->prepare('UPDATE items SET stok = :stok, ambang_minimum = :ambang, status = :status, catatan = :catatan WHERE id = :id AND poli = "gigi"');
                $stmt->execute([
                    ':stok'   => $stok,
                    ':ambang' => $ambang,
                    ':status' => $status,
                ...
                $stmt = $pdo->prepare('SELECT stok, ambang_minimum FROM items WHERE id = :id AND poli = "gigi"');
                $stmt->execute([':id' => $id]);
                $row = $stmt->fetch();
        ...
        header('Location: dashboard-poli-gigi.php?' . http_build_query(['status' => $flashStatus, 'msg' => $flashMsg]));
        exit;
    }
    ...
    // =========================================================
    $stmt = $pdo->query('SELECT COALESCE(SUM(stok), 0) AS total FROM items WHERE poli = "gigi"');
    $row = $stmt->fetch();
    if ($row) $totalStok = (int) $row['total'];

    $stmt = $pdo->query('SELECT COUNT(*) AS jumlah FROM items WHERE poli = "gigi" AND stok <= ambang_minimum');
    $row = $stmt->fetch();
...
<title>Admin — Dashboard Poli Gigi</title>
...
<aside id="sidebar" class="h-screen w-64 fixed left-0 top-0 flex flex-col py-lg px-md z-50" style="background:linear-gradient(180deg,#0f5238,#0b3e2c);">
  ...
  <div class="logo-row flex items-center gap-md mb-xl px-xs">
    ...
    <div class="sidebar-label overflow-hidden whitespace-nowrap">
      ...
      <p class="text-[12px] text-white/60">Poli Gigi</p>
    </div>
  ...
  <nav class="flex-1 space-y-1">
    <a class="nav-link ripple-container relative overflow-hidden flex items-center gap-md py-2.5 px-md rounded-xl text-primary font-bold bg-white shadow-card transition-all duration-200" href="dashboard-poli-gigi.php" title="Dashboard">
    ...
    <a class="nav-link ripple-container relative overflow-hidden flex items-center gap-md py-2.5 px-md rounded-xl text-white/80 hover:bg-white/10 hover:text-white transition-all duration-200" href="input-barang-gigi.php" title="Input Barang">
...
<header id="topbar" class="fixed top-0 right-0 left-0 h-16 bg-surface/85 backdrop-blur-md z-30 flex justify-between items-center gap-md px-margin-mobile sm:px-margin-desktop ml-64 border-b border-outline-variant/50">
  <div class="flex items-center gap-md flex-1 min-w-0">
    ...
    <div class="relative w-full max-w-md search-box">
      ...
      <input id="search-input" class="w-full pl-10 pr-md py-2 bg-surface-container-low border-none rounded-full focus:ring-2 focus:ring-primary/25 text-[14px] placeholder:text-outline" placeholder="Cari obat atau alat kesehatan gigi…" type="text">
    </div>
  </div>
...
<main id="main-content" class="ml-64 pt-24 pb-16 px-margin-mobile sm:px-margin-desktop min-h-screen">
  <div class="mb-lg flex flex-col sm:flex-row sm:justify-between sm:items-end gap-md">
    <div>
      ...
      <p class="text-on-surface-variant text-[14px] sm:text-[15px]">Pantau ketersediaan logistik medis Poli Gigi secara real-time.</p>
    </div>
    <a href="input-barang-gigi.php" class="flex items-center justify-center gap-sm bg-primary text-white px-lg py-3 rounded-xl font-bold text-[14px] hover:bg-primary-container active:scale-[0.98] transition-all shadow-card w-full sm:w-auto">
  ...
  <div class="animate-card bg-surface-container-lowest p-lg rounded-2xl border border-outline-variant/60 shadow-card hover:shadow-card-hover transition-shadow relative overflow-hidden mb-8" style="animation-delay:.02s">
  ...
  <div class="bg-surface-container-lowest rounded-2xl border border-outline-variant/60 shadow-card overflow-hidden">
    <div class="p-lg border-b border-outline-variant/60 flex flex-col md:flex-row md:items-center justify-between gap-md">
      <h3 class="font-display text-[17px] font-bold text-on-surface">Daftar Inventaris Poli Gigi</h3>
...
<div id="modal-edit" class="modal-hidden fixed inset-0 z-[65] flex items-center justify-center p-4">
  ...
  <form method="POST" action="dashboard-poli-gigi.php" class="modal-panel relative bg-surface-container-lowest rounded-2xl shadow-card-hover w-full max-w-sm p-lg">
...
<div id="modal-delete" class="modal-hidden fixed inset-0 z-[65] flex items-center justify-center p-4">
  ...
  <form method="POST" action="dashboard-poli-gigi.php" class="modal-panel relative bg-surface-container-lowest rounded-2xl shadow-card-hover w-full max-w-sm p-lg text-center">
...
<script>
  ...
  function downloadLaporan() {
    ...
    window.open('laporan-pdf-gigi.php?' + params.toString(), '_blank');
  }
  ...
  window.addEventListener('DOMContentLoaded', () => {
    ...
    if (window.history.replaceState) {
      window.history.replaceState({}, document.title, 'dashboard-poli-gigi.php');
    }
    <?php endif; ?>
  });
</script>
